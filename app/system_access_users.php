...
.access-tabs{display:flex;gap:8px;flex-wrap:wrap}.access-tabs a{min-width:90px;text-align:center}
...
.access-tags{display:flex;gap:4px;flex-wrap:wrap;margin-top:5px}.access-tag{display:inline-block;padding:2px 6px;border-radius:999px;background:rgba(127,127,127,.14);font-size:.72rem}.access-cell-actions{margin-top:8px;display:flex;gap:6px;align-items:center;flex-wrap:wrap}.access-read-error{display:block;margin-top:4px;font-size:.72rem;color:var(--muted);max-width:260px;white-space:normal}
</style>
<div class="page-title">
    <div><h1>System → Access → Users</h1><p>Compare local OPNsense users across managed firewalls.</p></div>
    <div class="access-tabs"><a class="button" href="/system_access_users.php">Users</a><a class="button secondary" href="/system_access_groups.php">Groups</a><a class="button" href="/system_access_ssh_key.php">Add SSH Key</a></div>
</div>
<div class="alert warningbox"><strong>Safe SSH-key action available.</strong> Use <em>Add SSH Key</em> to add one public key to one existing user on one firewall without changing any other user property. Full user editing remains authentication-critical.</div>
...
<?php foreach ($userNames as $name): ?><tr><td class="object-col"><strong><?= h($name) ?></strong></td><?php foreach ($fleet as $fid=>$entry): $user=$entry['users'][$name]??null; $agent=$agentsByFirewall[$fid]??null; $agentVersion=is_array($agent)?trim((string)($agent['last_version']??'')):''; $writable=is_array($agent)&&(int)($agent['enabled']??0)===1&&$agentVersion!==''&&version_compare($agentVersion,'0.1.10','>='); ?><td class="access-cell"><?php if (!$entry['ok']): ?><span class="badge bad">Read failed</span><small><?= h((string)($entry['error'] ?? 'Unknown read error')) ?></small><?php elseif (!is_array($user)): ?><span class="badge neutral">Missing</span><?php else: ?><span class="badge good">Present</span><?php if ($user['disabled']): ?> <span class="badge bad">Disabled</span><?php endif; ?><small><?= h((string)($user['description'] ?: 'No description')) ?></small><?php if ($user['uid']!==''): ?><small>UID <?= h((string)$user['uid']) ?></small><?php endif; ?><small>Shell: <?= h((string)($user['shell'] ?: 'Default / none')) ?></small><small><?= $user['has_password'] ? 'Password configured' : 'No local password' ?><?= $user['otp'] ? ' · OTP configured' : '' ?><?= !empty($user['has_authorized_keys']) ? ' · SSH key(s) configured' : '' ?></small><?php if ($user['groups']): ?><div class="access-tags"><?php foreach ($user['groups'] as $group): ?><span class="access-tag">group: <?= h($group) ?></span><?php endforeach; ?></div><?php endif; ?><?php if ($user['privileges']): ?><div class="access-tags"><?php foreach ($user['privileges'] as $priv): ?><span class="access-tag"><?= h($priv) ?></span><?php endforeach; ?></div><?php endif; ?><div class="access-cell-actions"><a class="button secondary" href="/system_access_user_edit.php?firewall_id=<?= (int)$fid ?>&amp;user=<?= rawurlencode($name) ?>">Edit</a><?php if (!$writable): ?><small><?= $agentVersion!==''?'Agent '.$agentVersion.' · 0.1.10+ required':'Agent 0.1.10+ required for write' ?></small><?php endif; ?></div><?php endif; ?></td><?php endforeach; ?></tr><?php endforeach; ?>
</tbody></table></div>
