...
$capable = $online && $version !== '' && version_compare($version, '0.1.15', '>=');
...
<div class="card ssh-key-card">
    ...
    <?php if (!$capable): ?>
        <div class="alert warningbox"><strong>Cannot deploy yet.</strong> This firewall needs an online agent 0.1.15.</div>
    <?php else: ?>
    <form method="post" action="/system_access_ssh_key_action.php">
        ...
        <label>OpenSSH public key
            ...
            <small>Exactly one public key. If the same key already exists, the agent verifies the actual authorized_keys file and makes no config change.</small>
        </label>
