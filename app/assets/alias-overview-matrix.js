window.opnSentralAliasOverviewMatrix = function(options){
    ...
    async function renameEntry(button){
        ...
        if(!/^[A-Za-z0-9_]+$/.test(trimmed)){ window.alert('Alias name may contain only letters, numbers and underscores.'); return; }

        const allIds = matchingFirewallIds(oldName);
        let targetIds = [firewallId];
        if(allIds.length > 1){
            const renameAll = window.confirm(
                'Rename alias "' + oldName + '" to "' + trimmed + '" on all ' + allIds.length + ' firewalls where it exists?\n\n' +
                'OK = all matching firewalls\nCancel = only ' + firewallName
            );
            targetIds = renameAll ? allIds : [firewallId];
        }else if(!window.confirm('Rename alias "' + oldName + '" to "' + trimmed + '" on ' + firewallName + '?')) return;
    ...
    async function deployMissing(button){
        ...
        try{
            ...
            const response = await fetch('/alias_deploy_missing.php', {method:'POST', credentials:'same-origin', body:payload});
            const raw = await response.text();
            let data; try{ data = JSON.parse(raw); } catch(error){ throw new Error(raw.replace(/\s+/g,' ').slice(0,700)); }
            if(!response.ok || data.ok !== true) throw new Error(data.error || 'Alias deployment failed.');
            const failures = (data.results || []).filter(item => !item.ok);
    ...
    function render(data){
        ...
        firewalls.forEach(function(result){
            ...
            items.forEach(function(item){
                ...
                if(!key) return;
                if(!aliasMap.has(key)){
                    aliasMap.set(key, {name:String(item.name || ''), description:String(item.description || ''), type:String(item.type || ''), byFirewall:new Map(), hasManaged:false, hasUnmanaged:false});
                }
        ...
        const aliases = Array.from(aliasMap.values()).sort((a,b) => a.name.localeCompare(b.name, undefined, {numeric:true,sensitivity:'base'}));
        summary.innerHTML = firewalls.length + ' firewalls · ' + reachable + ' reachable · ' + aliases.length + ' aliases · ' + placements + ' placements · ' +
        ...
        const head = firewalls.map(function(result){
            ...
            return '<th class="alias-matrix-firewall"><strong>' + escapeHtml(fw.name) + '</strong>' +
                '<a class="muted alias-matrix-firewall-url" href="' + escapeHtml(fw.base_url) + '" target="_blank" rel="noopener">' + escapeHtml(fw.base_url) + '</a></th>';
        }).join('');

        const rows = aliases.map(function(alias){
            const existingSources = firewalls.filter(result => result.aliases_ok && alias.byFirewall.has(Number(result.firewall.id))).map(result => ({result:result,item:alias.byFirewall.get(Number(result.firewall.id))}));
            const source = existingSources.find(entry => entry.item && entry.item.managed) || existingSources[0] || null;
            const missingTargets = firewalls.filter(result => result.aliases_ok && !alias.byFirewall.has(Number(result.firewall.id))).map(result => Number(result.firewall.id));
            const deployButton = source && missingTargets.length
                ? '<button type="button" class="button alias-deploy-missing" data-name="' + escapeHtml(alias.name) + '" data-source-firewall-id="' + Number(source.result.firewall.id) + '" data-source-firewall-name="' + escapeHtml(source.result.firewall.name) + '" data-target-firewall-ids="' + missingTargets.join(',') + '">Apply to missing (' + missingTargets.length + ')</button>'
                : '';

            const cells = firewalls.map(function(result){
                if(!result.aliases_ok) return '<td class="alias-matrix-cell"><span class="badge bad">Unavailable</span></td>';
                const item = alias.byFirewall.get(Number(result.firewall.id));
                if(!item) return '<td class="alias-matrix-cell alias-matrix-missing"><span class="muted">—</span></td>';
                return '<td class="alias-matrix-cell">' +
                    '<div class="alias-matrix-cell-status">' + statusBadge(item) + '</div>' +
                    '<div class="alias-matrix-cell-meta">' + (item.enabled ? '<span class="badge good">Enabled</span>' : '<span class="badge neutral">Disabled</span>') + '</div>' +
                    '<button type="button" class="button secondary inventory-rename alias-matrix-rename" data-name="' + escapeHtml(item.name) + '" data-firewall-id="' + Number(result.firewall.id) + '" data-firewall-name="' + escapeHtml(result.firewall.name) + '">Rename</button></td>';
            }).join('');

            return '<tr class="alias-matrix-row" data-has-managed="' + (alias.hasManaged ? '1' : '0') + '" data-has-unmanaged="' + (alias.hasUnmanaged ? '1' : '0') + '">' +
                '<th scope="row" class="alias-matrix-name"><strong>' + escapeHtml(alias.name) + '</strong>' +
                ...
                (deployButton ? '<div class="alias-matrix-row-action">' + deployButton + '</div>' : '') + '</th>' + cells + '</tr>';
        }).join('');

        list.innerHTML = '<div class="card alias-matrix-card"><div class="table-scroll alias-matrix-scroll"><table class="management-table alias-matrix-table">' +
            '<thead><tr><th class="alias-matrix-corner">Alias</th>' + head + '</tr></thead><tbody>' + rows +
        ...
        list.querySelectorAll('.alias-deploy-missing').forEach(button => button.addEventListener('click', () => deployMissing(button)));

        const failures = firewalls.filter(result => !result.aliases_ok);
        if(failures.length){ errorBox.textContent = failures.map(result => result.firewall.name + ': ' + result.aliases_error).join(' | '); errorBox.classList.remove('hidden'); }
