...
try {
    if ($firewallId <= 0) throw new RuntimeException('Select one firewall.');
    ...
    if (!in_array($type, $allowed, true) || $blob === '' || base64_decode($blob, true) === false) {
        throw new RuntimeException('Unsupported or invalid OpenSSH public key.');
    }
    ...
    if ($version === '' || version_compare($version, '0.1.11', '<')) throw new RuntimeException('Agent 0.1.11 or newer is required.');
...
header('Location: /system_access_ssh_key.php');
